...
const style = computed(() => ({
  backgroundImage: `url(${
    props.redPacketType === 1
      ? "@/assets/images/red-packet.png"
      : "@/assets/images/red-packet-blin.png"
  })`,
}));
...
async function handleReject() {
  ...
  // 第二个红包金额为0不弹出红包
  if (!Number(props.redPacketConfig?.value2)) {
    return;
  }
  setTimeout(() => {
...
async function handleConfirm() {
  ...
  await updateTestOrder({
    testOrderId,
    redPacketStatus: props.redPacketType == 1 ? 1 : 3, //  1 解锁报告 3 领取报告
  });
}
// 退出
