<template>
  <div v-if="isDataLoaded">
    <div class="answer" v-if="payStyleType == 0 || !payStyleType">
      <div
        class="head-title"
        :style="{ background: class_type == 1 ? '#6EA9FB' : '#6EA9FB' }"
      >
        本次测评已为您抵扣
...
import { testOrderPay, checkTestOrderPay } from "@/api/order";
export default {
  ...
  setup() {
    ...
    // const num = parseInt(str.substring(0, str.indexOf('.')));
    async function init() {
    ...
    async function handlePay(fay) {
      ...
      try {
        ...
        const redirectUrl = `${window.location.origin}/PaymentReminder2?${search}`;
    ...
    function onPayModalCancel() {
      if (hasRedPacket.value) {
        ...
        if (!state.redPacketType) {
          state.redPacketType = 2;
          // sessionStorage.setItem("price_type", 1);
        }
      }
    }
