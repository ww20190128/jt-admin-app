  ...
  setup() {
    ...
    async function loadTestPaperInfo() {
      if (promotionId) {
        ...
        // 获取订单信息
        const { testPaperInfo, promotionInfo, lastTestOrderInfo } =
          await getTestPaperInfo({ promotionId, testPaperId });
        paper_son_id.value =
          data.goods_info?.paper_id == 0 ? paper_id : data.goods_info?.paper_id;
    ...
    async function init() {
      ...
      if (!testOrderId) {
        ...
        Object?.assign(data, response);
        if (response?.testOrderInfo && response?.testOrderInfo?.id) {
          state.lastTestOrderInfo = response.testOrderInfo;
        }
      }
      const {
        questionList, //  题目
      ...
      document
        ...
        ?.setAttribute("content", testPaperInfo?.subtitle);
