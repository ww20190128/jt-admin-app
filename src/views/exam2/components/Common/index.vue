...
import Steps from "@/views/exam/components/Common/components/Steps2";
...
export default {
  ...
  setup() {
    ...
    // const themeVars = {
    //   vanActionBarButtonDangerColor: "#ff5d7d",
    // };
    // http://192.168.0.35:7000/exam?guid=61b72dd4&puid=655acebc&hasParams=1
    //https://xlm.whyuanshi.com/exam?puid=655acebc&guid=61b72dd4&hasParams=1
    ...
    async function handleOption(index) {
      ...
      if (currentOptionAnswer.value) {
        const val = state.answerList.find((v) => v.id === current.value.id);
        val.answer = index;
      } else {
    ...
    async function handleLookReport() {
      ...
      if (state.testPaperInfo?.ageSet && (state.testPaperInfo?.ageSet).length) {
        goChooseYear();
      } else {
        handleReport();
      }
    ...
    async function loadTestPaperInfo() {
      if (promotionId) {
        ...
        // 获取订单信息
        const { testPaperInfo, lastTestOrderInfo } = await getTestPaperInfo({
          promotionId,
          testPaperId,
        });
    ...
    async function handleReport() {
      ...
      try {
        ...
        if (!testOrderId) {
          ...
          const { needPay } = await checkTestOrderPay({ testOrderId }); // 检查是否需要付费
    ...
    function checkStatus() {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        try {
          const { testComplete, needPay } = await checkTestOrderPay({
            testOrderId,
          });
          if (testComplete == 1 && needPay === 0) {
            // 完成测试并且不需要支付
            isStop.value = false;
            router.replace({
              path: "/report",
    ...
    async function init() {
      ...
      if (state.testPaperInfo.answerTimeLimit > 0) {
        time.value = state.testPaperInfo.answerTimeLimit * 60 * 1000;
      }
  ...
  .exam-list {
    ...
    .item-list {
      ...
      .item2 {
        ...
        box-shadow: 3px 3px 0 #a77d0d, 3px 3px 0 0.5px #225d97;
