<template>
      ...
      <div class="center" :style="{ height: !item2.fay ? '' : height + 'px', overflow: !item2.fay ? '' : 'hidden' }"
        ...
        <div :id="'content' + (index + 1)">
          ...
          <ResultName :title="item2.weidu_name" :result="item2.weidu_result?.name" :color="item2.extend.tubiao_color"
            titleColor="#5a5a5a" />
          <PositionBar :value="item2.last_percent" :color="item2.extend.tubiao_color" />
  ...
  <div v-else>
    ...
    <PositionBar :value="currentResult.last_percent" :color="currentResult.extend.tubiao_color" />
    ...
    onMounted(async () => {
      ...
      props.data.weiduList.forEach((item, index) => {
        ...
        if (element) {
          ...
          // item.height =parseInt(height< parseInt(height2) ? parseInt(height2) : height / 3 + 300)
          // console.log('%c [  item.height ]-178', 'font-size:13px; background:pink; color:#bf2c9f;',  item.height)
