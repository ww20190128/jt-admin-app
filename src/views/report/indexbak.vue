<template>
    <template v-if="!error">
      <div v-if="!loading">
        <div class="report" :class="{
          ...
          <ReportHead :testPaperInfo="testPaperInfo" :userInfo="userInfo" :testOrderInfo="testOrderInfo">
          </ReportHead>
  
          <div class="record-top" @click="handleGoErrorRecord" v-if="errorRecordShow && errorNum > 0 && buttonVisible">
            <div class="fixed">
              <van-icon name="warning" class="icon" /> 您一共有{{
                errorNum
              }}道错题，点击立即查看！
            </div>
          ...
          <!-- <div
            v-if="errorRecordShow && !errorNum > 0 && reportInfo.ruiwen_ord?.length !== 0  && buttonVisible"> -->
          <!-- <div class="record-top" @click="handleGoErrorRecord"
            v-if="errorNum <= 0 && errorRecordShow && reportInfo.ruiwen_ord?.length !== 0 && goods_sale_price_type == 3 && buttonVisible">
            <div class="fixed">
              <van-icon name="success" color="#fff" />恭喜您，本次测试全部答对！
            </div>
            <div class="placeholder"></div>
          </div> -->
          ...
          <div class="gradient-header" v-if="buttonVisible">
            <div class="title">{{ testPaperInfo.name }}报告</div>
            <div class="time">{{ reportInfo.create_time }}</div>
            <!-- 瑞文 -->
            <div class="button-list" v-if="reportInfo.ruiwen_ord?.length !== 0 && goods_sale_price_type == 3">
              <div class="button-item button-wight" @click="handleReset">免费重测</div>
              <div class="button-item button-wight" @click="exportPtf">下载报告</div>
              <div class="button-item button-wight" @click="handleGoErrorRecord">错题解析 </div>
            </div>
            <div class="button-list"
              v-else-if="reportInfo.ruiwen_ord?.length !== 0 && (goods_sale_price_type == 1 || goods_sale_price_type == 2)">
            </div>
            <!-- 其他 -->
            <div class="button-list" v-else>
              <div class="button-item" @click="handleCollect">收藏结果</div>
              <div class="button-item" @click="handleReset"> 免费重测</div>
              <div class="button-item" @click="handleGive">赠送好友</div>
              <div class="button-item" @click="pushBaseUrl('/home')">更多评测</div>
            </div>
  
          </div>
  
          <div class="result-wrapper" ref="posterRef1">
            <div class="result" v-if="reportInfo.duoweidu &&
            ...
            <div class="result" v-else>
              <!-- 总结果计分 -->
              <ZJGJF :data="reportInfo.total_result_scoring" v-if="reportInfo.total_result_scoring &&
                reportInfo.total_result_scoring?.length !== 0
              " />
              <!-- 总结果计分2 -->
              <ZJGJF :data="reportInfo.total_result_scoring2" v-if="reportInfo.total_result_scoring2 &&
                reportInfo.total_result_scoring2?.length !== 0
              " />
  
              <!-- 无规则说明 -->
              <WGZSM :data="reportInfo.wuguize" v-if="reportInfo.wuguize && reportInfo.wuguize?.length !== 0" />
  
              <!-- 多维度网状 -->
              <DWDWZ :data="reportInfo.duoweidu_mesh" v-if="reportInfo.duoweidu_mesh &&
                reportInfo.duoweidu_mesh?.length !== 0
              " />
              <!-- 瑞文 -->
  
              <RW :data="reportInfo" v-if="reportInfo.ruiwen_ord?.length !== 0 || reportInfo.ruiwen_adv?.length !== 0" />
              <div class="bottom-rw" v-if="reportInfo.ruiwen_ord?.length !== 0">
                <LargeTitle :label="reportInfo.ruiwen_ord?.setting?.title2"
                  :icon="reportInfo.ruiwen_ord?.setting?.title_icon_tag2" />
  
                <RichTextBody :data="reportInfo.ruiwen_ord?.ruiwenOrdInfo?.zhiye_tuijian" />
  
                <ul class="job-list">
                  <li v-for="( item, index ) in reportInfo.ruiwen_ord?.professionInfo?.job " :key="index">
                    {{ item }}</li>
                </ul>
  
                <Warning :data="reportInfo.ruiwen_ord?.setting?.zhuyi2" title="注意事项" icon="fa vaaeq fa-bullhorn" />
              </div>
              <!-- 单维度 -->
              <WDS :data="reportInfo.danweidu" v-if="reportInfo.danweidu && reportInfo.danweidu?.length !== 0
              " />
              <!-- 多维度 -->
              <WDM :data="reportInfo.duoweidu" v-if="reportInfo.duoweidu &&
                reportInfo.duoweidu?.length !== 0 &&
                reportInfo.duoweidu?.setting?.duo_weidu_type == 1
              " />
              <!-- MBTI排列状 -->
              <!-- v-if="reportInfo.mbti_pl && reportInfo.mbti_pl?.length !== 0" -->
              <MBTIPLZ :data="reportInfo.mbti_pl" v-if="reportInfo.mbti_pl && reportInfo.mbti_pl?.length !== 0"
                :testOrderInfo="testOrderInfo" :sendCouponList="sendCouponList" />
  
              <!-- MBTI网状 -->
              <MBTIWZ :data="reportInfo.mbti_wz" v-if="reportInfo.mbti_wz && reportInfo.mbti_wz?.length !== 0" />
              <!-- 计分排列题 -->
              <JFPLT :data="reportInfo.jifen_pl" v-if="reportInfo.jifen_pl && reportInfo.jifen_pl?.length !== 0
              " />
  
              <!-- 番外结果 -->
              <FWJG :data="reportInfo.fanone" v-if="reportInfo.fanone && reportInfo.fanone?.length !== 0" />
              <!-- 番外网状 -->
              <FWWZ :data="reportInfo.fantwo" v-if="reportInfo.fantwo && reportInfo.fantwo?.length !== 0" />
              <!-- 番外维度 -->
              <FWWD :data="reportInfo.fanthree" v-if="reportInfo.fanthree && reportInfo.fanthree?.length !== 0
              " />
  
              <!-- 多层分页建议 -->
              <DCFYJY :data="reportInfo.duocheng_page" v-if="reportInfo.duocheng_page &&
                reportInfo.duocheng_page?.length !== 0
              " />
  
              <div class="rw_report"
                v-if="(reportInfo.ruiwen_ord?.length !== 0 || reportInfo.ruiwen_adv?.length !== 0) && payStyleType3SendGoods.length !== 0 && buttonVisible">
                <div class="report_title">
                  <div class="report_title_span1"><i class="fa fa-gift" style="margin-right: 5px;" />您的测评套餐包含</div>
                </div>
                <div class="report_item wrongQuestion" v-if="errorRecordShow && payStyleType3SendGoods.isShowCuoti == 1"
                  @click="handleGoErrorRecord">
                  <div class="top_conter">
                    <div class="title">
                      <div class="baweijiedu2 wrongQuestion_title">智商测试错题解析</div>
                      <div class="baweijiedu3 wrongQuestion_center">包含错题列表，正确选项和答题详解</div>
                    </div>
  
                    <div class="bottom_conter bottom_jx">
                      查看解析
                    </div>
                  </div>
                </div>
                <div class="report_item " v-for="( item, index ) in payStyleType3SendGoods?.goodsList " key="index"
                  :class="index == 0 ? 'advancedIntelligence' : 'qingshang'" @click="evaluation(item)">
                  <div class="top_conter">
                    <div class="title">
                      <div class="baweijiedu2 " :class="index == 0 ? 'advancedIntelligence_title' : 'qingshang_title'">
                        {{ item.goods_name }}</div>
                      <div class="baweijiedu3 " :class="index == 0 ? 'advancedIntelligence_center' : 'qingshang_center'">
                        {{ item.goods_subtitle }}</div>
                    </div>
                    <div class="bottom_conter " :class="index == 0 ? 'bottom_zs' : 'bottom_qs'">
                      开始测试
                    </div>
                  </div>
                </div>
                <div class="report_title">
                  <div class="report_title_span2"><i class="fa fa-exclamation-circle"></i>
                    请先关注微信公众号，在公众号里可免费测试，公众号：{{ wechatConf?.name }}
                  </div>
                </div>
  
              </div>
              <!-- 扩展阅读 -->
              <KZYD :data="reportInfo.extend_read" v-if="reportInfo.extend_read &&
                reportInfo.extend_read?.length !== 0
              " ref="posterRef6" />
            </div>
            ...
            <div class="recommend" v-if="buttonVisible">
              ...
              <CellCardList :data="recommendList" :clickItem="handleClickGoodsItem" />
            ...
            <!-- 评价 -->
            <div class="evaluation-content" v-if="(!hasComment || isWarning) && buttonVisible" ref="posterRef2">
              ...
              <template v-if="!hasComment">
                <div class="comment">
                  ...
                  <div class="evaluation">
                    ...
                    <div class="textarea">
                      <van-field v-model="form.content" rows="2" autosize type="textarea" placeholder="其他建议反馈，可以写在这里" />
                    </div>
        ...
        <WeChatQr v-model:show="wechatQrVisible" :title="qrModalTexts.title">
          <template #subhead>
            <p class="subhead"
              :class="{ 'subhead-mini': qrModalTexts.subheadMini, 'subhead-text': qrModalTexts.subheadText }">
              {{ qrModalTexts.subhead }}
            </p>
            <p class="subhead2" :class="{ 'subhead-mini': qrModalTexts.subheadMini }">
          ...
          <template #button>
            ...
            <div class="popupButton"
              v-if="(reportInfo.ruiwen_ord?.length !== 0 || reportInfo.ruiwen_adv?.length !== 0) && goods_sale_price_type == 3"
              @click="close">{{ qrModalTexts.buttonText }}</div>
        ...
        <BaseDialog v-model:show="resetShow" :showConfirmButton="false">
          <div class="dialog-content">
            ...
            <div class="report-item" v-for="( item, index ) in resetOrderList " :key="index">
              <div class="left">
                {{ $filters.formatDate(item.createTime) }}{{ index === 0 ? "初测" : "重测" }}
              </div>
              <div v-if="item.status === 2" class="right" @click="handleCheckReport(item)">
                查看报告
              </div>
        ...
        <GiveFriendDialog v-model:show="giveVisible" @confirm="handleGiveConfirm">
          <div class="give-content">
            ...
            <div class="text">
              ...
              }}次免费赠送的权益，现在还剩{{ userInfo?.vipInfo?.vipGiveLimit - userInfo?.vipInfo?.vipGiveNum }}次。
            </div>
          </div>
          <template #button>
            <span v-if="userInfo?.vipInfo?.vipGiveLimit > userInfo?.vipInfo?.vipGiveNum">立即免费赠送</span>
        ...
        <VipDialog v-model:show="vipOpenVisible">
          ...
          <template #text>
            温馨提示：
            <span class="give-warning">VIP会员，无法享受赠送权益</span>
          </template>
          <template #bottom>
        ...
        <div class="float-right record" v-if="errorRecordShow && errorNum > 0 && buttonVisible"
          @click="handleGoErrorRecord">
          <van-icon name="records" class="icon" />
          错题集
        </div>
  
        <div class="float-right gift" @click="handleGift"
          v-if="sendCouponList.length && (reportInfo.mbti_pl?.length == 0 || reportInfo.mbti_pl.setting.pl_type != 7) && buttonVisible">
          <van-icon name="point-gift-o" class="icon" />
          领取现金礼包
        </div>
  
        <div class="footer-bar"
          v-if="sendCouponList.length && buttonVisible && (reportInfo.ruiwen_ord?.length == 0 || reportInfo.ruiwen_adv?.length == 0) && payStyleType3SendGoods.length == 0">
          <footer class="bar">
        ...
        <div class="bottomMenu"
          v-if="(reportInfo.ruiwen_ord?.length !== 0 || reportInfo.ruiwen_adv?.length !== 0) && goods_sale_price_type == 2 && buttonVisible">
  
          <div class="menu">
            <div class="menu-item1" v-if="payStyleType3SendGoods?.goodsList[0]"
        ...
        <!-- 瑞文高档 -->
        <div class="bottomMenu"
          v-if="(reportInfo.ruiwen_ord?.length !== 0 || reportInfo.ruiwen_adv?.length !== 0) && goods_sale_price_type == 3 && buttonVisible">
          <div class="menu">
          ...
          <div class="menu">
            <div class="menu-item2" v-if="payStyleType3SendGoods?.goodsList[1]"
        ...
        <!-- 版本选择 -->
        <OptionsSelect v-model:show="optionVisible" v-model:current="optionCurrent"
          :versionConfig="testPaperInfo?.versionConfig" @confirm="handleOptionConfirm" />
      </div>
    </template>
  
    <Error v-else>
      <template #footer>
        提示：如果多次刷新仍无法显示，可以关注公众号：{{ wechatConf?.name }}，点击底部菜单我的测评，查看结果。
      </template>
    </Error>
  </template>
  ...
  import KZYD from "./typesComponents/KZYD";
  ...
  import {
    createResetTestOrder,
    getResetTestOrderList
  } from "@/api/testPaper";
  import {
    checkTestOrderPay
  } from "@/api/order";
  ...
  export default {
    ...
    setup() {
      ...
      async function initErrorRecord() {
        if (state.errorRecordShow) { // 获取错题数量
      ...
      async function init(testOrderId) {
        ...
        try {
          ...
          state.errorRecordShow =
            Number(reportInfo?.cuoti_jiexi?.is_show) === 1;
      ...
      function close() {
        if ((state.reportInfo.ruiwen_ord?.length !== 0 || state.reportInfo.ruiwen_adv?.length !== 0) && state.goods_sale_price_type == 3) {
          window.location.href = state.customerServiceLink
        } else {
      ...
      async function handleReset2() {
        if (!isWeChat) {
          if (state.reportInfo.ruiwen_ord?.length !== 0 && state.goods_sale_price_type == 3) {
            window.location.href = state.customerServiceLink
          } else {
            handleReset()
          }
      ...
      function downloadImg(params, name) {
        ...
        html2Canvas(shareContent, ops).then(canvas => {
          ...
          console.log('%c [ context ]-1361', 'font-size:13px; background:pink; color:#bf2c9f;', context)
      ...
      function getElementContent(elementId, imgName) {
        ...
        let h = parseInt(window.getComputedStyle(elementId).height) - state.pnHeight
        ...
        let name = state.testPaperInfo.name + '报告'
        setTimeout(function () {
          new html2Canvas(elementId, {  //关键在于new
            useCORS: true,
      ...
      function shotPic() {
        ...
        domtoimage
          ...
          .then((dataUrl) => {
            console.log('%c [ dataUrl ]-1598', 'font-size:13px; background:pink; color:#bf2c9f;', dataUrl)
            //输出图片的Base64,dataUrl
            // 下载到PC 
      ...
      async function handleGiveConfirm() {
        if (userInfo.value.vip_send_num - userInfo.value.vip_use_send_num <= 0) {
      ...
      function handleClickGoodsItem({ testPaperId }) {
        pushBaseUrl({
          path: "/detail",
          query: {
            testPaperId,
  ...
  .record-top {
    ...
    .placeholder {
      width: 100%;
      height: 36px;
      background: linear-gradient(90deg, #ffecd2 0%, #f9e1bd 100%);
    }
  }
